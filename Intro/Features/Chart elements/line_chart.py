...
line_chart = pd.DataFrame(var_dic_data_1, index= (1,2,3,4,5,6))

st.dataframe(line_chart,hide_index=False,width=700)
...
st.line_chart(line_chart, x="Score 1",y="Score 2",color="#ffaa00")
...
# syntax 
# st.line_chart(data=None, *, x=None, y=None, color=None, width=0, height=0, use_container_width=True)
st.subheader("2: Example of st.line_chart")
...
line_chart = pd.DataFrame(var_dic_data_1, index= (1,2,3,4,5,6), columns=["Score 1", "Score 2", "Score 3"])

st.dataframe(line_chart,hide_index=False,width=700)

st.subheader("Dont Specify only three columns")
st.line_chart(line_chart, color=["#fd0", "#f0f", "#04f"])
...
st.subheader("3: Example of st.line_chart. Scores generated randomly")
...
st.line_chart(chart_data, x="Score 1", y="Score 2", color="Score 3")
